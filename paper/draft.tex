...
Libraries are written by authors with different education backgrounds, quality standards, and, most importantly, needs:
  it's not unusual for a weekend project solving a local need to end up as a critical dependency among several open source projects~\cite{cheapcomplexity}.
...
% FIXME: Garbage collection
  worse even, the fact that a library changes one of its dependencies can cause severe performance pathologies to upstream projects---that is, while an interface remains the same due to an intermediate layer of abstraction, performance characteristics can change dramatically~\cite{jest}.
...
Pathological inputs reaching \ttt{minimatch}, even if benign, can cause significant performance degradation~\cite{redos} deep in the dependency chain that can also affect other parts competing for the same resource~\cite{evp}.
Developers use various techniques to understand such problems---for example, they collect and replay traces against off-line versions of the system, or use statistical profiling to identify hot code-paths.
These techniques, however, require significant \emph{manual} effort:
  capturing traces, setting up testbeds, replaying traces, analyzing statistics, and debugging performance are all tedious and time-consuming tasks, compounded by the difficulties of mapping the results to the right third-party libraries.

Having the ability to quickly detect a slowdown and attribute it to a bottlenecked component is key, by toggling on/off a profiling analysis that operates at library boundaries.
The analysis would wrap library interfaces with logic that generates a model of the current workload.
Profiling can operate at a high resolution in time and space:
  (i) at every function call entering a library, and (ii) on hundreds of libraries across an application.
but does not need to track detailed operations such as direct variable accesses.
...
Each library wrapper would collect profiling statistics at its own boundary, updating a global structure ordering the libraries' overall needs.
...
Extracting typing information is helpful in a variety of ways, as type safety is an important property helpful in variety of scenarios. 
%and types are lightweight annotations that can be quickly and efficiently checked.
For example, types can be used to (i) identify program invariants to be preserved during code modifications, (ii) generate (machine-checkable) program documentation, or (ii) guide program synthesis and regeneration~\cite{daikon}.
This is especially problematic in dynamic programming languages:
  in the absence of static type signatures, there is not much one can say about a object \ttt{o} without executing it and introspecting its properties.

As a case in point, consider the \ttt{gRPC} library for serializing and de-serializing objects~\cite{grpc}.
Given a value returned from a library call---say, a bignum or crypto library---a developer has to first run the library manually, take note of the result, and then write a protocol buffer specification file declaring the type; %, common in distributed systems
...
%   we expand on in the discussion section~\sx{reqs}.
...
Fig.~\ref{fig:ex2}\emph{a--d} exemplifies our transformation techniques in the context of JavaScript.
...
% \eat{ is \ttt{A}-internal and how much}
comes from \ttt{B}. % or \ttt{B}'s dependency, \ttt{C}.
