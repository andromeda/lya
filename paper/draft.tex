...
To address these challenges, we propose a novel approach to dynamic analysis, targeting dynamic programming languages---and implemented in \sys for the server-side JavaScript ecosystem (>1.3M libraries~\cite{modulecounts}). 
...
The overall approach has several practical benefits over prior work:
  ...
  (iii) it features low runtime performance overheads, enabling the potential for toggling its use in production environments.
...
This section details our implementation of \sys for server-side JavaScript in about 1.5K lines of code on top Node.js v8.9.4~\sx{impl1}, and 
...
We now expand on the requirements bolting our techniques on other runtime environments.
...
The set of techniques presented in this paper shines when there is a need for urgency, select online analysis, and a high level of abstraction.
...
\sys's set of techniques is \emph{not appropriate} for monolithic programs or ones written in low-level languages such as C.
...
\nv{
\sys builds on aspect-oriented programming (AOP), a programming model where program points map to actions taken at these points~\cite{aop}.
Both approaches lower complexity by mapping multiple points across the program to the same actions, but \sys does not alter the program's control flow.
Moreover, AOP works on languages that were already designed to support it, whereas \sys is explicitly designed to bolt onto existing languages.
}
...
We present \sys, a novel library-aware analysis framework for dismantling, analyzing, and reassembling programs by combining name shadowing, context wrapping, and transformation of the underlying dependency graph.
