...
\sys's  development achieves goals~\sx{goals}, demonstrating that it is indeed feasible to build a frameworks that 
  operates as a library around an existing runtime,
  does not require learning a new language and tools, and 
  gets meaningful information by operating only at library boundaries, 
...
% At a high level, we are interested in three questions:
  ...
  (iii) an evaluation of \sys's behavior using our analyses on realistic, multi-library applications.
...
Analyses expressed in \sys are about 4$\times$ smaller than equivalent analyses written for a heavy-weight instrumentation frameworks.
\sys supports high-performance analysis at the boundaries of individual libraries, adding on average less than \fixme{2$\times$}.
Finally, \sys seems to scale well to large applications with hundreds of libraries while interposing on several thousands of objects.
...
For example, to better understand PD and confirm it works correctly, we craft a micro-benchmark with four dependencies of known static bottlenecks.
...
Table~\ref{tab:synthetic} depicts the results of the three analysis applied to a subset of our benchmarks.
Not all analysis-benchmark combinations are useful---\ie some modules do not feature interesting access or performance patterns.
Currently, these benchmarks return the results we expect, but they did help with the identification of \sys's bugs.
The \ttt{delay.js} is the PD benchmark mentioned earlier, with static bottlenecks.
Initial loading time, affected by transformations altering the environment, is not affected (delta is under 1ms).
...
~~~-t2                         & 19.17  & 59.77 &  60.75 &  57.15  \\
~~~-t3                         & 21.64  & 205.2 &  205.53&  132.25 \\
~~~-t4                         & 20.69  & 59.70 &  60.75 &  59.92  \\
~~~-t5                         & 20.49  & 70.48 &  72.20 &  64.71  \\
Moment~\cite{moment}           & 31.73  & 48.04 &  49.23 &  44.95  \\
yargs~\cite{yargs}             & 13.42  & 13.00 &  14.41 &  13.47  \\
...
To understand \sys's performance characteristics at the level of individual modules, we perform two experiments.

In the first experiment, we collect a diverse set of eight popular libraries from npm, 
% used by thousands of other projects,
on the order of hundreds of lines of code.
As a workload, we use the project-internal test suite that comes bundled with each library (invocable by \ttt{npm test});
  \ttt{minimist} involves several distinct test harnesses, which we 
These tests stress functionality as expected by the authors of the library, and confirms compatibility with all of its functionality.
We apply \sys using the three analyses we developed, and compare the results with their baseline performance (Tab.~\ref{tab:meso}).
The results show that executions augmented with analyses are within 2$\times$, averaging \fixme{XX\%}.
Different benchmarks put a different pressure on \sys's wrappers---some resulting in the invocation of as many as \fixme{XXX} wrappers, as shown by the resulting analysis output on disk.
...
To be fair in our comparison, we design a simple analysis from scratch that both frameworks can support---namely, one for tracking counts of global variable accesses~\sx{impl2}.
The core of the analysis requires a few lines, but expressing it in \sys requires 4$\times$ fewer lines of code;
  the effort of employing \sys as a library was significantly lower than Jalangi2, for which we \fixme{relied} on a Docker container to simplify the framework's setup.
...
We note here that Jalangi2 has a runtime performance that is significantly faster than the classic Jalangi, as it eschews record/replay capabilities.
Jalangi2 is more powerful, expressing analyses that are more fine-grained than \sys's, but results in higher developer effort and runtime performance.
...
\caption{
  \footnotesize{
    ...
    The table shows the overheads of applying the three analyses to three large applications.
  }
}
...
Finally, we perform end-to-end program dialysis  on three large, popular applications:
  KoaBlog~\cite{koa}, a minimal blogging platform built on the Koa framework,
  Modeda~\cite{moeda}, a CLI converter that, among other features, uses online currency exchange rates to convert currencies from the terminal, and
  Terminalizer~\cite{terminalizer}, an application for recording terminal sessions to generating animated gif images and shareable a web player links.
These are popular applications---for example, Koa.js and Terminalizer have 28.3K and 8.7K stars stars on GitHub, respectively.
Their structural and performance characteristics are shown in Tab.~\ref{tab:macro}.

The difference between the modules detected statically (by running \ttt{npm ls}) and the ones observed by \sys is due to a few reasons.
One is that \sys does not report accesses if a library has not been accessed, whereas \ttt{npm} traverses reports on the static library structure.
Additionally, \sys analyzes modules that are part of the standard library (some of which have themselves internal dependencies),  not shown by \ttt{npm}.

Applying \sys to these applications generates much larger analysis results, while the performance of analyzed code is on average still within 2--3$\times$ over the baseline.
The size of the resulting JSON files raises the challenge of analysis interpretability---surprisingly, just running Terminalizer with \ttt{--help} returns more than 6K entries from several libraries.
Our routines for post-processes \sys's results also counts by module name---\ie squashes duplicate reports by human-name, not module identifier.
...
It also squashes cache entries that correspond to one module imported by multiple modules, which would create separate entries in \sys's augmented cache, but would result in a single count for Tab.~\ref{macro} (for \sys's treatment of the module cache, see~\S\ref{three}).
