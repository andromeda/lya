...
Results show that \sys delivers up to an order of magnitude performance improvements over current state-of-the-art dynamic analysis systems, compatibility with over 50 libraries and applications, and support for a range of dynamic analyses, each implemented in under 100 lines of code.

% operate at the library  interface level  
% This approach occupies an new point in the design trade-off space, enabling lya to operates at the library interface level in unmodified production..


% transform and bind their context prior to their evaluation.
% expose instrumentation opportunities to developers.
% This mechanism enables a new lightweight instrumentation technique.

% 
% 
% 
% different
% dynamic interposition techniques
% 
% We show how to exploit the technique to deliver very efficient interposition schemes
% 
%   the new technique operates at a finer granularity, 
%   coarse granularity
% 
% we can exploit our characteristics.
% Allows us to do X. This in turn has the following benefits for the developer Y:
% 
% the results show a way faster
% 
% What are the benefits
% we present a library-oriented dynamic analysis technique and system
% technique the functionality for interposing at library boundaries
% 
% 
% We do dynamic analys 10 times faster and has
% 
...
These libraries are highly popular, averaging \todo{4842488} weekly downloads (total: \todo{227596957}) and are depended upon by about \todo{656} other packages or applications on average (total: \todo{30812}).
...
Jalangi is a popular dynamic analysis framework for JavaScript;
  it provides fine-grained instrumentation by executing the program using a custom interpreter.
...
In terms of programs, we use Jalangi's extensive test suite.
This is not entirely by choice:
  Jalangi's interaction with the bare-bones libraries was hitting important parsing issues, and was further complicated by \ttt{npm test} (which is an external program outside Node, but tightly coupled with it).
It is important


(Fig.~\ref{fig:meso}).
We compare with and compare \sys's performance to that of Jalangi2.
...
\todo{Two types: zooming into prior benchmarks, and targeted micro packages}
...
The primary source of overhead seems to be the use of JavaScript's \ttt{with} construct~\sx{}, 
The \ttt{with} construct dominates overheads, as it (i) interposes on too many accesses, only a fraction of which are relevant, and (ii) remains significantly unoptimized, since its use is strongly discouraged by the JavaScript standards~\cite{}.
% Proxy objects exhibit a much lower cost.
% \TODO{Add some numbers from \ttt{t19} for different configs}.
...
\begin{figure}
  ...
  % \includegraphics[width=0.18\textwidth]{./figs/lya_ctx_unique_invalid}
  % \includegraphics[width=0.18\textwidth]{./figs/lya_ctx_total_invalid}
  \caption{
    \textbf{Access characteristics.}
    \todo{Explain}
  }
  \label{fig:objects}
\end{figure}
...
\begin{figure}[b!]
  ...
  % \includegraphics[width=\columnwidth]{./figs/lya_unique_invalid}
  \includegraphics[width=\columnwidth]{./figs/lya_total_valid}
  % \includegraphics[width=\columnwidth]{./figs/lya_total_invalid}
  \caption{
    \textbf{Depth vs. permissiveness.}
    From the top:
    (a) number of \sys wrappers,
    (b) unique valid accesses,
    (c) unique invalid accesses,
    (d) total valid accesses, and
    (e) total invalid accesses---all as a function of \todo{enforcement} depth.
  }
  \label{fig:objects}
\end{figure}
