...
Contrary to other types of analysis, its key benefits make it the primary (or only) candidate in a variety of scenarios---namely, ones that require:
  (i) current runtime information, such as profiling information, in view of changing workloads or use patterns~\cite{staticdynamic},
  (ii) accurate visibility into the execution, devoid of abstraction or approximation~\cite{staticdynamic},
  (iii) Turing-complete policies, where invariants themselves are arbitrary programs~\cite{contracts1, contracts2, contracts3},
  (iii) dynamic or runtime-reflection features~\cite{jsanalysis1, jsanalysis2}, such as the ones present in Python and JavaScript.
...
Unfortunately, reaping these benefits comes with significant costs in terms of (often, both) developer effort and runtime performance.
Instrumenting the program often requires a good understanding of its internals.
Applying associated tools adds to this requirement, such as instrumentation framework, instruction sets, and other underlying primitives~\cite{pin, valgrind}.
Modifying the runtime environment of the program is also cumbersome, error-prone, and JavaScript's V8 engine.
Problem-specific solutions require significant investment outside the language's mental model---for example, performance tracing frameworks such as perf and dtrace each feature their own language for specifying tracing programs, different from the application's language.
---as a case in point, Jalangi reports XX overhed (exact overheads always depend on the specific analysis).

Both overheads are severely compounded by the use of third-party libraries, development-time constructs usually glued together without a full understanding of their internals.
Libraries are used pervasively nowadays to lower development costs and improve software quality, a recent phenomenon enabled by zero-cost of sharing:
% This over-reliance to third-party libraries is a recent phenomenon, 
  with the proliferation of language-specific package repositories, the effort of publishing and that of using a library have essentially dropped to zero.
As a result, applications today
  to the point that it is common for thousands of applications to depend on libraries that count only a few dozen lines:
  for example, leftpad and
  
This paper proposes a novel approach to dynamic analysis, termed \emph{module dialysis}\footnote{
  A method of deconstruction, often for the purposes of removing waste products or altering piecewise components.
  From ancient Greek diá ($\delta\iota\alpha$ ``inter-, through'') and lúein (``loosen''); synchronically, dia- + -lysis.
} and implemented in \sys.
Library dialysis is a general technique, applicable to any programming language, that enables dynamic fragmentation, analysis, and re-assembly of applications at the level of individual libraries. 
It operates during program execution, automating wrapping and transforming the underlying dependency graph, allowing developers to extract meaningful insights with only a few lines of analysis-specific code
By operating at the granularity of libraries, module dialysis has several benefits over full-fledged dynamic analysis:
  (i) it allows bolting library dialysis itself as a library onto existing runtime environments that do not feature dynamic analysis,
  (ii) it supports analysis expressions directly in the same language, tools, and abstractions as the host application language---eschewing low-level instrumentation, and
...
To demonstrate the effectiveness of the proposed techniques, we build a prototype of library dialysis, called \sys, targeting the JavaScript ecosystem---featuring more than 1M libraries (cite), high code-reuse (cite), and a pressing need for usable dynamic analysis (cite).
We develop a diverse set of four analyses that infer practical information about the analyzed program---including an allow/deny security analysis, a profiling analysis showing hot modules, a synthesis-oriented analysis on input-output data, and an analysis enforcing a static union-based type system at runtime.
This set of analysis drives the design requirements for \sys and highlights the benefits of bolt-on module-aware dynamic analysis.
Finally, we we use \sys to evaluate these analyses on three different levels:
  (i) synthetic micro-benchmarks that highlight certain features and confirm the correctness of our analyses in a controlled environment;
  (ii) single-module meso-benchmarks that allow comparison with more traditional dynamic analysis frameworks; and
  (iii) end-to-end full-application macro-benchmarks that show behavior of the \sys system on realistic workloads. 
...
\item \S\ref{impl} discuss our prototype implementation, \sys, as a pluggable library for the JavaScript ecosystem;
\item \S\ref{eval} implementation and evaluation of the four analyses, showing \sys can express insightful analyses in 10–20 lines of code, adds a virtually imperceptible increase in the load latency of individual modules, and scales to real applications with hundreds of modules.
\end{itemize}
