...
NodeProf takes a different approach than code-level instrumentation, statically instrumenting the AST, which makes it easier to maintain compared to source changes.
Similar to \sys, NodeProf 
is not tied to the source code, has significantly lower overheads, and can be toggled on/off during program execution.
Requires AST extraction and manipulation to build, which is not available in many languages % it is an additional requirement
and is in conflict with many dynamic features. 
It stands at a different point in the design space---more powerful analysis, but with significantly more effort:
  assumes knowledge of the internal Graal and Truffle APIs and requires recompilation of the Java hooks.

Similar to NodeProf, \sys 

Aran~\cite{javascript1} is presented in a poster to 
