...
Dynamic program analysis is a long-standing technique for getting insights into the execution of a program.
This paper presents a new form of dynamic analysis, termed \emph{dialysis}, enabled by the fact that modern dynamic languages feature an import mechanism that loads code at runtime as a string.
Dialysis works by coupling light static transformations with more substantial dynamic ones, inserting hooks for capturing a library's full observable behavior.
...
Dialysis offers a new point in the dynamic-analysis trade-off space:
  as it operates at a coarse granularity and does not modify the language runtime environment, it offers better performance and compatibility but foregoes expression-level visibility.
To evaluate its potential, we implement dialysis in LYA and apply it on targets drawn from the JavaScript ecosystem.
Our results show $9.6\times$ performance improvements over state-of-the-art dynamic analysis, compatibility with over 50 libraries and applications, and analyses expressed in under 100LoC---all while offering useful insights on three realistic use-case analyses.
